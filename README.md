...
## Environment
The Lambda function has no external dependencies other than Python 2.7. It requires access to the AmazonEC2ReadOnlyAccess managed policy. It can be configured with an optional environment variable (CHECK_REFRESH_SECONDS) to control the cash refresh rate. By default, CHECK_REFRESH_SECONDS = 60 seconds.
